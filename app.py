...
class StrategicReport(FPDF):
    def header(self):
        ...
        self.cell(0, 10, 'CONFIDENTIAL // STRATEGIC CONTRACT ASSESSMENT', 0, 1, 'R')
    ...
    def footer(self):
        ...
        self.cell(0, 10, f'Page {self.page_no()} | Generated by Contract Intelligence AI', 0, 0, 'C')
    ...
    def chapter_title(self, label):
        ...
        self.line(10, self.get_y(), 200, self.get_y()) # Underline title
        self.ln(5)
    ...
    def chapter_body(self, body):
        ...
        # Markdown Parsing for clean PDF
        lines = body.split('\n')
        for line in lines:
            line = line.strip()
            ...
            if line.startswith('## '):
                ...
                self.multi_cell(0, 6, chr(149) + " " + line[2:].strip()) # Bullet char
            else:
                self.set_font('Arial', '', 11)
                self.set_text_color(50, 50, 50)
...
def generate_pdf(data):
    ...
    # 2. Executive Verdict Box (Stylish)
    pdf.set_fill_color(240, 245, 255) # Very Light Blue
    pdf.set_draw_color(0, 51, 102) # Navy Border
    pdf.rect(10, pdf.get_y(), 190, 45, 'FD')
    
    pdf.set_xy(15, pdf.get_y() + 5)
    pdf.set_font('Arial', 'B', 12)
    pdf.set_text_color(0, 51, 102)
    pdf.cell(0, 8, f"STRATEGIC VERDICT: {risk.get('level', 'N/A').upper()} RISK ({risk.get('score', 0)}/100)", 0, 1)
    
    pdf.set_font('Arial', '', 11)
    pdf.set_text_color(0, 0, 0)
    pdf.set_x(15)
    # Combine bullets for the box
    summary_text = ""
    ...
    # 3. Commercial Snapshot (Grid-like)
    pdf.chapter_title("1. Commercial Snapshot")
    pdf.set_font('Arial', 'B', 11)
    pdf.cell(50, 8, "Contract Type:", 0, 0); pdf.set_font('Arial', '', 11); pdf.cell(0, 8, str(comm.get('contractType')), 0, 1)
    pdf.set_font('Arial', 'B', 11)
    pdf.cell(50, 8, "Value Structure:", 0, 0); pdf.set_font('Arial', '', 11); pdf.cell(0, 8, str(comm.get('value')), 0, 1)
    pdf.set_font('Arial', 'B', 11)
    pdf.cell(50, 8, "Duration:", 0, 0); pdf.set_font('Arial', '', 11); pdf.cell(0, 8, str(comm.get('duration')), 0, 1)
    pdf.ln(5)
...
def main():
    ...
    if uploaded_file:
        if st.button("🚀 Run Strategic Analysis"):
            with st.spinner("⚙️ Analyzing Commercials, Risk & Compliance..."):
